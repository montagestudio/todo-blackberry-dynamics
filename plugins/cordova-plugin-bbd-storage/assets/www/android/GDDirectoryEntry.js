...
;(function() {
  ...
  GDDirectoryEntry.prototype.getDirectory = function(path, options, successCallback, errorCallback) {
    ...
    var win = typeof successCallback !== 'function' ? null : function(result) {
      ...
      var entry = new GDDirectoryEntry(result.name, result.fullPath, fs);
    ...
    success = function(entry) {
      if (entry) {
        if (typeof successCallback === 'function') {
          ...
          var fs = new (require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem');
          var result = new GDDirectoryEntry(entry.name, entry.fullPath, fs);
    ...
    success = function(entry) {
      if (entry) {
        if (typeof successCallback === 'function') {
          // create appropriate Entry object
          var fs = new (require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem');
          var result = (entry.isDirectory) ? new GDDirectoryEntry(entry.name, entry.fullPath, fs) :
