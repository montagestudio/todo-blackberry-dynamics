          ...
          success = function(entry) {
              if (entry) {
                  if (typeof successCallback === 'function') {
                      ...
                      var result = new (require('cordova-plugin-bbd-storage.FileEntry'))(entry.name, entry.fullPath, fs);
                      try {
          ...
          success = function(entry) {
              if (entry) {
                  if (typeof successCallback === 'function') {
                      // create appropriate Entry object
                      var fs = new (require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem');
                      var result = (entry.isDirectory) ? new (require('cordova-plugin-bbd-storage.DirectoryEntry'))(entry.name, entry.fullPath, fs) : new (require('cordova-plugin-bbd-storage.FileEntry'))(entry.name, entry.fullPath, fs);
                      try {
                          successCallback(result);
                      }
                      catch (e) {
