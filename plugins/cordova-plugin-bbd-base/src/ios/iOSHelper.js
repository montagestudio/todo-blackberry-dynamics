...
function PlistHelper(context) {
    try {
        ...
        this.projectName = new ConfigParser(path.join(context.opts.projectRoot, 'config.xml')).name();
        this.platformFiles = fs.readdirSync(this.platformRoot);
        this.originalNameOfiOSProject = this.getOriginalNameOfiOSProject();
        this.plistFilePath = path.join(this.platformRoot, this.originalNameOfiOSProject, this.originalNameOfiOSProject + "-Info.plist");
        this.plistObj = this.convertPlistToObj(this.plistFilePath);
        this.entitlementPlistFilePath = path.join(this.platformRoot, this.originalNameOfiOSProject, "Resources", "Entitlements.plist");
        this.entitlementPlistObj = this.convertPlistToObj(this.entitlementPlistFilePath);
        this.packageName = new ConfigParser(path.join(context.opts.projectRoot, 'config.xml')).packageName();
    } catch (e) {
        throw e;
    }
}
...
PlistHelper.prototype = {
    ...
    editEntitlementPlist: function() {
        ...
        fs.writeFileSync(this.entitlementPlistFilePath, plist.build(this.entitlementPlistObj), { encoding: 'utf8' });
    },
    ...
    removeUrlScheme: function(schemeName) {
        if (this.plistObj.CFBundleURLTypes[0]) {
            this.plistObj.CFBundleURLTypes[0].CFBundleURLSchemes = this.plistObj.CFBundleURLTypes[0].CFBundleURLSchemes.filter(function(value) {
...
function XcconfigHelper(context) {
    try {
        ...
        this.defaultXcconfigPath = path.join(this.context.opts.projectRoot, 'plugins', 'cordova-plugin-bbd-base', 'src', 'ios', 'default.xcconfig');
        this.cordovaBuildXcconfigPath = path.join(this.context.opts.projectRoot, 'platforms', 'ios', 'cordova', 'build.xcconfig');
...
XcconfigHelper.prototype = {
    editXcconfig: function() {
        ...
        if (this.cordovaBuildXcconfigContent.indexOf('CODE_SIGN_ENTITLEMENTS') > 0) {
            this.cordovaBuildXcconfigContent = this.cordovaBuildXcconfigContent.replace('CODE_SIGN_ENTITLEMENTS', '// CODE_SIGN_ENTITLEMENTS');
        ...
        if (fs.existsSync(this.defaultXcconfigPath)) {
            var comment = '// Build configuration for BlackBerry Dynamics SDK for iOS. Please do not remove it!!!',
                contentForAdding = this.defaultXcconfigContent.substring(this.defaultXcconfigContent.indexOf(comment) - 2, this.defaultXcconfigContent.length);
